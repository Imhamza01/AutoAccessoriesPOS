...
async def create_brand(
    ...
    try:
        ...
        return {"success": True, "message": "Brand created successfully", "brand": brand}
    except Exception as e:
        logger.error(f"Failed to create brand: {e}")
        raise HTTPException(status_code=500, detail=str(e))

# ==================== STOCK ENDPOINTS (must be before /{product_id}) ====================
...
# ==================== BULK OPERATIONS (must be before /{product_id}) ====================
...
# ==================== SEARCH (must be before /{product_id}) ====================
...
async def search_products(
    ...
    try:
        ...
        return {"success": True, "products": products, "count": len(products), "search_term": q}
    except Exception as e:
        logger.error(f"Failed to search products: {e}")
        raise HTTPException(status_code=500, detail=str(e))

# ==================== REPORTS (must be before /{product_id}) ====================
...
# ==================== CODE LOOKUP (must be before /{product_id}) ====================
...
# ==================== FOR-POS (must be before /{product_id}) ====================

@router.get("/for-pos", dependencies=[Depends(require_permission("products.view"))])
async def get_products_for_pos(
    ...
    try:
        from repositories.product_repo import get_product_repository
        repo = get_product_repository()
        result = repo.get_all_products({'is_active': True}, 1, 9999)
        return {"success": True, **result}
    except Exception as e:
...
async def get_products(
    ...
    try:
        ...
        if out_of_stock: filters['out_of_stock'] = True
        repo = get_product_repository()
...
# ==================== PRODUCT BY ID (dynamic — must be last among GET routes) ====================
...
async def adjust_product_stock(
    ...
    try:
        ...
        return {"success": True, "message": "Stock adjusted successfully", **result}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
...
async def calculate_product_price(
    ...
    try:
        ...
        return {"success": True, "price_calculation": result}
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
